package ucacue.edu.ec.sofia.controller.api;
...
import ucacue.edu.ec.sofia.dto.*;
import ucacue.edu.ec.sofia.persistence.entity.*;
import ucacue.edu.ec.sofia.service.GenericCRUDService;


import javax.validation.Valid;

import static ucacue.edu.ec.sofia.common.util.Constants.API_DOC_ANEXO_1;
...
public class OperacionesController {
    ...
    @ApiOperation(value = "Devolver un la libreta  ")
    @GetMapping(value = "{id}/libreta", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    public ResponseEntity<byte[]> getLibreta(
            @Valid @ApiParam(value = API_DOC_ANEXO_1, required = true) @PathVariable("id") Integer id
    ){


        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_TYPE, "application/json; charset=UTF-8");
        return (new ResponseEntity<byte[]>(seviceTransacion.generarReporte(id), headers, HttpStatus.OK));
    }
